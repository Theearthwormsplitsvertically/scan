...
> **历史总体规格，部分内容已被 v0.4.0 取代。** 当前正式 CLI 以 [Nmap 风格扫描器命令设计](./2026-08-13-nmap-style-cli-design.md) 和仓库 README 为准：使用 `-<module>`、组合模块参数、全量 `scan` 与 `-output`，不支持本文旧章节中的 `asset-agent <module> scan`、`-o` 或 `--output-dir`。本文的动态注册表、Provider、证据协议和阶段路线仍作为背景设计保留。
>
> 本文描述的 CMDB ACK、保留清理、常驻调度、事件增量和积压保护仍是未实现规划，不是当前发行版能力，也不得作为当前功能的验收依据。

日期：2026-08-13  
状态：历史总体规格；现行行为以 v0.4.0 README 与后续专项规格为准
