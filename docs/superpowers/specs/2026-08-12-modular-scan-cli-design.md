> **历史规格，已被替代。** 当前行为基线是 [2026-08-13 可扩展资产扫描器设计](./2026-08-13-extensible-asset-scanner-design.md)。本文中的 `scan <module>` 前缀语法、固定模块表、`socket` 聚合语义和协议 1.0 单文件输出不再是当前实现目标，仅供追溯早期决策。

# Linux 资产 Agent 模块化扫描设计

## 扫描前置画像（当前行为基线）

所有 `scan` 命令在业务模块之前只执行一次只读画像：读取发行版与版本、内核、架构、init、cgroup、安全组件、容器运行时和可用数据源。模块选择不只依赖发行版名称，而以实际可用数据源为主。

随后由 Agent 编排层为所选模块生成 `CollectionStrategy`。策略记录模块、实际后端、理想数据源、缺失数据源、状态和降级原因；无法满足硬性数据源时不调用该模块采集器，并在报告中留下 `unsupported` 证据。完整报告与单模块报告都必须包含 `system_profile` 和 `strategies`。
