...
func TestNewRegistryContainsOnlyImplementedModulesAndPlansAll(t *testing.T) {
	...
	if !reflect.DeepEqual(listed, []string{"connection", "container", "host", "network", "package", "port", "process", "service"}) {
	...
	if got := moduleNames(plan); !reflect.DeepEqual(got, []string{"host", "network", "package", "process", "port", "service", "connection", "container"}) {
